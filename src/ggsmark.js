...
export default (text) => {
  ...
  let matchTextLeftExp = (/\:text-left/)
  let splitText = node.literal.split(matchTextLeftExp)
  ...
  for (let index in splitText) {
    if (index % 2 == 0) {
      ...
      let div = new Node('custom_block')
      div.onEnter = `<div style="text-align: left"> ${splitText[index]}`
      div.onExit = '</div>'
      node.insertBefore(div)
    }
  }
